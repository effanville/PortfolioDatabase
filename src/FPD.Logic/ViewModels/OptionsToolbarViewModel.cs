...
public sealed class OptionsToolbarViewModel : DataDisplayViewModelBase
{
    ...
    private async Task ExecuteNewDatabase()
    {
        ...
        if (result == MessageBoxOutcome.Yes)
        {
            ...
            OnModelUpdated(new PortfolioEventArgs(true));
        }
    }
    ...
    private async Task ExecuteLoadDatabase()
    {
        ...
        await _updater.PerformUpdate(ModelData, new UpdateRequestArgs<IPortfolio>(
            true,
            portfolio => loaded = _portfolioPersistence.Load(portfolio, options)));
    ...
    {
        var task = _updater.PerformUpdate(
        ModelData,
            new UpdateRequestArgs<IPortfolio>(
                true,
                async portfolio => await _portfolioDataDownloader.Download(portfolio)));
        await task;
        OnModelUpdated(new PortfolioEventArgs(Account.All));
    }
    ...
    private async Task ExecuteCleanData()
    {
        ...
        OnModelUpdated(new PortfolioEventArgs(Account.Security));
    }
    ...
    private void ExecuteRefresh()
        => OnModelUpdated(new PortfolioEventArgs(Account.All, true));
